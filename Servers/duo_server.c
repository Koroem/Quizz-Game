...
int main()
{
  ...
  printf("(Duo-Server) Waiting for games to start...\n\n");
  fflush(stdout);
    ...
    for (fd = 0; fd <= max_socket; fd++) /* Iterating over all the descriptors */
    {
      ...
      if (fd != sd && FD_ISSET(fd, &read_sockets)) // Executes only 1 time for each socket
      {
        ...
        memset(username, 0, strlen(username));
          ...
          if (users_data[index].nr_of_users == 2)
          {
            ...
            if (startgame == 2) // We can start the game
            {
              ...
              for (k = 0; k < users_data[index].nr_of_users; k++)
                write(users_data[index].users[k].socket_address, &startgame, sizeof(int)); // Informing the client the game is starting
...
void game(struct game local_game)
{
  ...
  for (user_path = 0; user_path < local_game.nr_of_users; user_path++)
    ...
    {
      if (write(local_game.users[user_path].socket_address, &user_data[user_path].score, sizeof(int)) <= 0) // Sending the score of the user
      {
        close(local_game.users[user_path].socket_address);
        local_game.users[user_path].flag = 0; // user disconnected, we change the flag to 
        printf("User %s has left game %d\n", local_game.users[user_path].username, local_game.idThread);
        continue;
      }
      ...
      for (int k = 0; k < local_game.nr_of_users; k++)
      {
        ...
        if (user_data[k].score == max_score)
        {
          ...
          if (write(local_game.users[user_path].socket_address, &user_length, sizeof(int)) <= 0) // Sending the username length of the winner
          {
            close(local_game.users[user_path].socket_address);
            local_game.users[user_path].flag = 0; // user disconnected, we change the flag to 0
          ...
          if (write(local_game.users[user_path].socket_address, &user_data[k].username, user_length) <= 0) // Sending the username of the winner
          {
            close(local_game.users[user_path].socket_address);
            local_game.users[user_path].flag = 0; // user disconnected, we change the flag to 0
